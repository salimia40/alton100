...
let supportScene = new Scene('eccountantScene')

supportScene.enter(async (ctx) => {
    ctx.reply('پیام خود را بنویسید این پیام به حسابدار ارسال و درصورت پاسخ از همین طریق پاسخ به دست شما خواهد رسید')
...
supportScene.on('message', async (ctx) => {
    ...
    if (owner == undefined) 
        owner = await User.findOne({role: config.role_owner})
